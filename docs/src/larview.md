...
> **Figure 1:** Visualization of the `LARLIB.jl` model `refectory` of type `Struct`, obtained by hirarchical aggregation of two cellular complexes `table` and `chair`.
...
[LARLIB.jl](https://github.com/cvdlab/LARLIB.jl) package: (a) 2D disk; (b) cylinder surface in 3D; (c) 2-sphere surface in 3D; (3) toroidal surface in 3D.
